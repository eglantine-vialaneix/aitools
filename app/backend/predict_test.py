...
def main():
    ...
    test_df = pd.read_csv(DATA_ROOT / "df_test.csv")
    features = [feature for feature in features if feature in train_df.columns and feature in test_df.columns]

    if not features:
        features = [
            column
            for column in train_df.columns
            if column not in {LABEL_COLUMN, "nom"} and column in test_df.columns
        ]
    ...
    result_df["régime_alimentaire_prédit"] = model.predict(test_df[features])
