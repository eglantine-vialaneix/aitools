...
export function DinoBucket({ children, className = "", isActive = false, style, ...props }: DinoBucketProps) {
  return (
    ...
    >
      ...
      <div className="relative z-10 grid h-full min-h-[348px] w-full grid-cols-5 auto-rows-min content-start gap-x-[10px] gap-y-[10px] overflow-hidden pl-[20px] pr-[60px] py-[20px]">
        {children}
      </div>
    </div>
  );
